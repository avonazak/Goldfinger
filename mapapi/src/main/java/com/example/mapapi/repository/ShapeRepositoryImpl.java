...
public class ShapeRepositoryImpl implements ShapeRepository {
    ...
    public Shape findOneByLatLng(Location location, ShapeType shapeType) {
        ...
        switch (shapeType)  {
            case Soil:
            {
                format = "SELECT json_build_object('polygons', ST_ASGEOJSON(dsmw.geom)::json," +
                        " 'polygonInfo', json_build_object('title', 'Soil type', 'description'," +
                        " concat('[\"faosoil ', dsmw.faosoil, ', domsoi ', dsmw.domsoi, '\",\"Country '" +
                        ", dsmw.country, '\",\"', ROUND(dsmw.sqkm,2)::text, ' sq. km.', '\"]')::json," +
                        "'color', dsmw.domsoi)) AS json FROM dsmw" +
                        " WHERE ST_CONTAINS(dsmw.geom, ST_MAKEPOINT(%.14f, %.14f));";
            }
                break;
            case Earthquake:
            {
